<template>
    <content>
        <form class="form-inline" role="form" method="post">
            <label class="control-label">联营商品IDs:</label>
            <div class="form-group">
                <input v-model="ids" type="text" class="form-control" placeholder="pkid">
            </div>
...
<script type="text/babel">
    ...
    export default {
        ...
        methods: {
            queryClick(){
                this.pIds = this.ids.split(" ")
                let params = {
                    pkids: this.pIds
                };

                util.postQesProxy("purchase/unionProductList", JSON.stringify(params)).done(data=> {
